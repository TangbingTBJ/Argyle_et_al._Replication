...
def mc(t):
    return t[0].upper() + t[1:]  # Capitalize first character, preserve the rest
...
def gen_variants(toks):
    ...
    for t in toks:
        ...
        for v in variants:
            transformed = v(t)
            token_transforms.append(transformed)
        
        for transformed in token_transforms:
            results.add(transformed)         # without leading space
            results.add(f" {transformed}")  # with leading space
...
#    for t in toks:
#        for v in variants:
#            results.append(v(t))      #( " " + v(t) )
#    return results  #output: [" donald", " DONALD", " Donald", " trump", " TRUMP", " Trump"]
...
def do_query( prompt, engine,max_tokens=2 ):
    messages = [
